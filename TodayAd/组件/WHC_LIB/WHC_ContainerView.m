...
- (void)setDelegate:(id<WHC_ContainerViewDelegate>)delegate{
    _delegate = delegate;
    _containerBar.homeDegate = (HomeViewController*)_delegate;
...
- (UIButton *)createButton:(CGRect)frame txt:(NSString *)txt tag:(NSInteger)tag{
    ...
    btn.layer.borderColor = RGBACOLOR(255, 68, 76, 1).CGColor;
    btn.layer.borderWidth = 0.5;
    btn.layer.masksToBounds = YES;
    [btn setTitle:txt forState:UIControlStateNormal];
    [btn setTitleColor:RGBACOLOR(255, 68, 76, 1) forState:UIControlStateNormal];
...
- (UIView *)createEditView{
    ...
    [editView addSubview:_editButton];
     rc = CGRectMake(KWHC_LableMargin + 90, (KWHC_ContainerBarHeight - KWHC_EditButtonHeight) / 2.0, 50, KWHC_EditButtonHeight);
    _allButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _allButton.frame = rc;
    _allButton.tag = 1000;
    _allButton.titleLabel.font = [UIFont systemFontOfSize:15];
    _allButton.layer.cornerRadius = KWHC_EditButtonHeight / 2.0;
    _allButton.layer.borderColor = [UIColor grayColor].CGColor;
    ...
    [_allButton addTarget:self action:@selector(clickEditViewButton:) forControlEvents:UIControlEventTouchUpInside];
    [editView addSubview:_allButton];
...
#warning 收起栏目
    ...
    if(tag == KWHC_DropButtonTag){
        if(!(sender.selected)){
            if (_delegate) {
                HomeViewController * ctl = (HomeViewController*)_delegate;
                [ctl.navigationController setToolbarHidden:NO animated:YES];
            }
