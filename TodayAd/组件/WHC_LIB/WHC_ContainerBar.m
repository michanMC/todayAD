...
+ (WHC_ContainerBarParam *)getWHC_ContainerViewParamWithTitles:(NSMutableArray*)titlesArr laterTitlesArr:(NSMutableArray *)laterTitlesArr{
    ...
    param.fontColor = [UIColor grayColor];
    param.focusFontColor = AppCOLOR;
    param.itemBarNBackgroudColor = [UIColor groupTableViewBackgroundColor];//[UIColor colorWithWhite:230.0 / 255.0 alpha:1.0];
...
- (void)initUILayout{
    ...
    _dropBtn.backgroundColor =[UIColor groupTableViewBackgroundColor];// [UIColor colorWithWhite:230.0 / 255.0 alpha:1.0];
...
- (void)clickDropBtn:(UIButton*)sender{
    if (_homeDegate) {
      //  HomeViewController *ctl = (HomeViewController*)_homeDegate;
        [_homeDegate.navigationController setToolbarHidden:YES animated:YES];
        NSLog(@"%@",_homeDegate);
...
- (void)rotateDropBtnDuring:(CGFloat)during{
    NSLog(@">>%@",_containerBarParam.titlesArr);
    for (NSString * str in _containerBarParam.titlesArr) {
        NSLog(@"==%@",str);
    }
    for (NSString * str in _containerBarParam.laterTitlesArr) {
        NSLog(@">>%@",str);
    }
